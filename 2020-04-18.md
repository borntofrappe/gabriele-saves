...
I created three visualizations to showcase the `d3-scale-chromatic` module. I ended up practicing with several other concepts, like a band scale from `d3-scale` and symbols from the `d3-shape` module , but the demo is purposed to show how the chromatic module allows to introduce color, and different color scales.

- scatter mind jokingly plots data points with different symbols and a limited sets of colors

- github contributions uses a continuous color scale
